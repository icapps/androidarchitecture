...
/**
 * Specialized [com.icapps.architecture.arch.MutableObservableFuture] which wraps a retrofit call and returns the body as well as the raw okhttp response.
 ...
 */
class RawRetrofitObservableFuture<T, O>(private val call: Call<T>,
                                        type: Type,
                                        private val nullableType: Boolean,
                                        private val headerInspector: ((Headers, T) -> T)?,
                                        private val transform: ((T) -> O)?,
                                        private val errorTransformer: ((ServiceException) -> Throwable)?) : ConcreteMutableObservableFuture<RawResponse<O>>() {
     ...
     */
    override fun observe(lifecycle: Lifecycle): RawRetrofitObservableFuture<T, O> {
        synchronized(lock) {
            if (cancelled)
                return this
     ...
     */
    override fun observe(onCaller: OnCallerTag): RawRetrofitObservableFuture<T, O> {
        synchronized(lock) {
            if (cancelled)
                return this
     ...
     * Starts the call on a background thread and delivers the results to the main thread
     */
    override fun observe(onMain: OnMainThreadTag): ConcreteMutableObservableFuture<RawResponse<O>> {
        synchronized(lock) {
            if (cancelled)
                return this
     ...
     */
    override fun execute(timeout: Long): RawResponse<O> {
        ...
        try {
            ...
            peekers.forEach { it(response) }
            peekBoths.forEach { it(response, null) }
            return response
        } catch (e: Throwable) {
            try {
                peekBoths.forEach { it(null, e) }
    ...
    private fun handleResponse(response: Response<T>): RawResponse<O> {
        if (response.isSuccessful) {
            if (isEmptyBody) {
                return doTransform(Unit as T, response.raw())
            } else if (isResponseBody) {
                return doTransform(if (headerInspector != null)
                    headerInspector!!(response.headers(), response.body() as T)
                else
                    response.body() as T, response.raw())
            } else {
    ...
    private fun doTransform(result: T, rawResult: okhttp3.Response): RawResponse<O> {
        @Suppress("UNCHECKED_CAST")
        return if (transform == null)
            RawResponse(rawResult, result as O)
        else
            RawResponse(rawResult, transform.invoke(result))
    }
